...
helm upgrade --install app-simulator $APP_SIMULATOR_CHART_DIR -f "$SCRIPT_DIR/receiver.yaml" -n $B_CLUSTER_NAMESPACE --set db.appSimulator.password=$POSTGRES_ADMIN_PASSWORD --set "imagePullSecrets={docker-registry-cred}" --set image.tag=$DOCKER_IMAGE_VERSION --set "db.appSimulator.namespace=$APP_SIMULATOR_DB_NAMESPACE" --set appSimulators.receiver.topicCreation.replicationFactor=3  --wait
