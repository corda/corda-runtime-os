...
class CryptoFlowOpsTransformerImplTests {
    ...
    fun `Should create query to filter my keys`() {
        ...
        assertTrue(
            query.keyIds.any {
                it!!.contentEquals(publicKeyFullIdFromBytes(keyEncodingService.encodeAsByteArray(myPublicKeys[0])))
            }
        )
        assertTrue(
            query.keyIds.any {
                it!!.contentEquals(publicKeyFullIdFromBytes(keyEncodingService.encodeAsByteArray(myPublicKeys[1])))
            }
        )
        assertTrue(
            query.keyIds.any {
                it!!.contentEquals(publicKeyFullIdFromBytes(keyEncodingService.encodeAsByteArray(notMyKey)))
            }
        )
