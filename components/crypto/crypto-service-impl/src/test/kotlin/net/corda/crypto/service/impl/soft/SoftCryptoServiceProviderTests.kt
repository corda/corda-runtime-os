...
class SoftCryptoServiceProviderTests {
    ...
    private fun createCryptoServiceProvider(): SoftCryptoServiceProviderImpl {
        val provider = SoftCryptoServiceProviderImpl()
        provider.persistenceFactory = InMemorySoftKeysPersistenceProvider()
        provider.start()
        return provider
    }

    private fun SoftCryptoServiceProviderImpl.createCryptoService(category: String): CryptoService = getInstance(
