...
create_cluster() {
  ...
  kubectl create secret generic kafka-credentials -n "${NAMESPACE}" \
    ...
    --from-literal=verification="${KAFKA_PASSWORDS_ARRAY[8]}"
...
export E2E_CLUSTER_B_REST_HOST=corda-rest-worker.$USER-$clusterB
export E2E_CLUSTER_B_REST_PORT=443
export E2E_CLUSTER_C_REST_HOST=corda-rest-worker.$USER-$clusterC
