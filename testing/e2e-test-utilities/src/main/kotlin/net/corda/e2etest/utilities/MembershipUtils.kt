...
fun ClusterInfo.registerStaticMember(
    ...
    cluster {
        println("QQQ registerStaticMember 1 $holdingIdentityShortHash ${Thread.currentThread().id}")
        assertWithRetry {
            interval(1.seconds)
            timeout(10.seconds)
            command {
                println("QQQ registerStaticMember 2 $holdingIdentityShortHash ${Thread.currentThread().id}")
                registerStaticMember(
                    ...
                    println("QQQ registerStaticMember 3 $holdingIdentityShortHash ${Thread.currentThread().id}")
                }
            }
            condition {
                it.code == ResponseCode.OK.statusCode
        ...
        assertWithRetry {
            ...
            interval(2.seconds)
            command {
                println("QQQ registerStaticMember 4 $holdingIdentityShortHash ${Thread.currentThread().id}")
                getRegistrationStatus(holdingIdentityShortHash) .also {
                    println("QQQ registerStaticMember 5 $holdingIdentityShortHash ${Thread.currentThread().id}")
                }
            }
            condition {
