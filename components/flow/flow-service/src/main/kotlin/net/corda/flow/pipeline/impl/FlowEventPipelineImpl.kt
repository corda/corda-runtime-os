...
import net.corda.flow.pipeline.FlowKillContext
...
class FlowEventPipelineImpl(
    ...
    override fun flowOperationalModeChecks(): FlowEventPipeline {
        val vnode = virtualNodeInfoReadService.get(context.checkpoint.holdingIdentity)
            ?: throw FlowFatalException(
                "Virtual node pipeline could not be created, virtual node ${context.checkpoint.holdingIdentity.shortHash} was not found."
            )
        if(vnode.flowOperationalStatus != "ACTIVE") {
            context.toBeKilled = FlowKillContext("Virtual Node's Flow Operational Status is ${vnode.flowOperationalStatus}.")
        }
        return this
    }
