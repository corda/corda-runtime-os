...
class AMQPSerializationFormatDecoder(
    ...
    private fun readObject(obj: Any?, registry: DynamicDescriptorRegistry, depth: Int, described: Boolean): Any? {
        return if (obj is DescribedType) {
            ...
            return list.map { readObject(it, registry, depth + 1, false) }.also {
            ...
            return obj.map { readObject(it, registry, depth + 1, false) }.toTypedArray().also {
                if (!described) referencedObjects.add(it)
            }
        } else {
            return obj
