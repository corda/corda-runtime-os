...
import net.corda.packaging.tests.legacy.DigestServiceImpl
...
class CpkTests {
    ...
    private val hashingService = DigestServiceImpl()

    private val cordaDevKey = hashingService.create("SHA-256" + ":" + System.getProperty("net.corda.dev.cert"))
    ...
    fun setup(@TempDir junitTestDir: Path) {
        ...
        workflowCpk = Cpk.Expanded.from(
            Files.newInputStream(workflowCpkPath),
            extractedWorkflowCpkPath,
            workflowCpkPath.toString()
        )
        ...
        workflowCpkLibraries =
            System.getProperty("net.corda.packaging.test.workflow.libs").split(' ').stream().map { jarFilePath ->
    ...
    companion object {
        private val DUMMY_HASH =
            Base64.getEncoder().encodeToString(DigestServiceImpl().getZeroHash(DigestAlgorithmName.SHA2_256).bytes)
    ...
    private fun tweakCordappJar(destination: Path, cordappJarTweaker: ZipTweaker) {
        object : ZipTweaker() {
            override fun tweakEntry(
                ...
                buffer: ByteArray
            ) = if (currentEntry.name == workflowCpk.cordappJarFileName) {
    ...
    fun `Verify hashes of jars in the lib folder of workflow cpk`() {
        for (entry in workflowCpk.libraryDependencies) {
            Assertions.assertEquals(
                workflowCpkLibraries[entry.key], entry.value,
    ...
    fun `Verify hash of main jar file`() {
        Assertions.assertEquals(
            computeSHA256Digest(Files.newInputStream(cordappJarPath)), workflowCpk.cordappHash,
    ...
    fun `Verify hash of cpk file`() {
        Assertions.assertEquals(
            computeSHA256Digest(Files.newInputStream(workflowCpkPath)), workflowCpk.cpkHash,
    ...
    fun `Verify that extracted files aren't corrupt and none of them is missing`() {
        ...
        Files.walk(referenceExtractionPath)
            .filter(Path::isRegularFile).forEach { filePath ->
                ...
                val extractedFile = extractedWorkflowCpkPath.resolve(relativePath)
                Assertions.assertTrue(
                    Files.exists(extractedFile),
                    "Missing file '$relativePath' from cpk extraction folder"
                )
                val extractedFileHash = computeSHA256Digest(Files.newInputStream(extractedFile), buffer)
                Assertions.assertEquals(
                    referenceFileHash,
                    extractedFileHash,
                    "$filePath differs from $extractedFileHash"
                )
            }
    }
    ...
    fun `Verify library files are correct`() {
        ...
        for (library in workflowCpk.libraries) {
            ...
            Assertions.assertEquals(
                libraryHash, workflowCpkLibraries[fileName],
    ...
    fun `Verify CPK dependencies are correct`() {
        ...
        Assertions.assertTrue(dependencies.size > 0)
        Assertions.assertTrue(dependencies.size == 1)
        val contractCpkDependency = dependencies.last()
        contractCpkDependency.apply {
            Assertions.assertEquals(
                System.getProperty("net.corda.packaging.test.contract.bundle.symbolic.name"),
                symbolicName
            )
            Assertions.assertEquals(System.getProperty("net.corda.packaging.test.contract.bundle.version"), version)
            Assertions.assertEquals(
                sortedSetOf(cordaDevKey), signers,
    ...
    fun `Verify cordapp signature`() {
        Assertions.assertEquals(
            ...
            workflowCpk.cordappCertificates
                .asSequence().map { it.publicKey.encoded.sha256() }.toSet()
        )
    }
    ...
    fun `allows a CPK dependencies file to list multiple signers`() {
        ...
        val hashdata1 = hashingService.hash(publicKey, DigestAlgorithmName.SHA2_384)
        val hashdata2 = hashingService.hash(publicKey, DigestAlgorithmName.SHA2_384)
        val hashdata3 = hashingService.hash(publicKey, DigestAlgorithmName.SHA2_512)
        val expectedSigners =
            sequenceOf(hashdata1, hashdata2, hashdata3).toCollection(TreeSet(Cpk.Identifier.secureHashComparator))
        ...
        |        <signers>
        |            <signer algorithm="${hashdata1.algorithm}">${
            String(
                Base64.getEncoder().encode(hashdata1.bytes)
            )
        }</signer>
        |            <signer algorithm="${hashdata2.algorithm}">${
            String(
                Base64.getEncoder().encode(hashdata2.bytes)
            )
        }</signer>
        |            <signer algorithm="${hashdata3.algorithm}">${
            String(
                Base64.getEncoder().encode(hashdata3.bytes)
            )
        }</signer>
        |        </signers>
        |    </cpkDependency>
