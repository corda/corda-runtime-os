    ...
    class OutboundMessageProcessor( //todo - if ttl expired - discard, marker (marker should remove) //will need epoch here
        ...
        private fun recordsForNewSession(state: SessionState.NewSessionNeeded): List<Record<String, *>> {
            ...
            return if(partitions.isEmpty()) {
                ...
                listOf(
                    Record(Schema.LINK_OUT_TOPIC, generateKey(), state.sessionInitMessage),
                    Record(Schema.SESSION_OUT_PARTITIONS, state.sessionId, SessionPartitions(partitions))
                )
            }
        }
                ...
                when (val payload = message.payload) {
                    is InitiatorHelloMessage -> {
                        val partitionsAssigned =
                            inboundAssignmentListener.getCurrentlyAssignedPartitions(Schema.LINK_IN_TOPIC).toList()
