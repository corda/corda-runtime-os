...
class VerificationRequestProcessorTest {
    ...
    private lateinit var deserializer: CordaAvroDeserializer<VerifyContractsResponse>
    ...
    fun setup(
        ...
        lifecycle.accept(sandboxSetup) { setup ->
            ...
            deserializer = setup.fetchService<CordaAvroSerializationFactory>(TIMEOUT_MILLIS)
                .createAvroDeserializer({}, VerifyContractsResponse::class.java)
    ...
    fun `successfully verifies transaction contracts`() {
        ...
        val response = processor.onNext(
            state = null,
            event = Record(TOPIC, REQUEST_ID, request)
        )

        assertThat(response).isNotNull
        assertThat(response.updatedState).isNull()
        assertThat(response.responseEvents).hasSize(1)

        val flowEvent = response.responseEvents.first().value as FlowEvent
        val externalEventResponse = flowEvent.payload as ExternalEventResponse
        assertThat(externalEventResponse.error).isNull()
        assertThat(externalEventResponse.requestId).isEqualTo(REQUEST_ID)

        val verifyContractsResponse = deserializer.deserialize(externalEventResponse.payload.array())!!
        assertThat(verifyContractsResponse.result).isEqualTo(VerificationResult.VERIFIED)
        assertThat(verifyContractsResponse.verificationFailures).isEmpty()
    }
    ...
    fun `unsuccessfully verifies transaction contracts`() {
        ...
        val verifyContractsResponse = deserializer.deserialize(externalEventResponse.payload.array())!!
        assertThat(verifyContractsResponse.result).isEqualTo(VerificationResult.INVALID)
        assertThat(verifyContractsResponse.verificationFailures.size).isEqualTo(1)

        val failure = verifyContractsResponse.verificationFailures[0]
        assertThat(failure.contractClassName).isEqualTo(TestContract::class.java.canonicalName)
        assertThat(failure.contractStateClassNames).isEqualTo(listOf(TestState::class.java.canonicalName))
        assertThat(failure.exceptionClassName).isEqualTo(IllegalArgumentException::class.java.canonicalName)
        assertThat(failure.exceptionMessage).isEqualTo(VERIFICATION_ERROR_MESSAGE)
    }

    @Test
    fun `returns redelivery state when CPK not available`() {
        val virtualNodeInfo = virtualNodeService.load(TEST_CPB)
        val holdingIdentity = virtualNodeInfo.holdingIdentity
        val cpksMetadata = cpiInfoReadService.get(virtualNodeInfo.cpiIdentifier)!!.cpksMetadata.filter { it.isContractCpk() }
        val cpkFileChecksums = cpksMetadata.mapTo(HashSet()) { it.fileChecksum }
        ...
        val processor = VerificationRequestProcessor(
            ...
            externalEventResponseFactory,
            maxRedeliveries = 1,
            redeliveryDelay = Duration.ofSeconds(0)
        )

        // Send request to message processor (no previous redeliveries)
        val response = processor.onNext(
            state = null,
            event = Record(TOPIC, REQUEST_ID, request)
        )
        ...
        assertThat(response).isNotNull
        assertThat(response.updatedState).isNotNull
        val resultState = response.updatedState
    ...
    @Test
    fun `keeps returning redelivery state when CPK not available`() {
        val virtualNodeInfo = virtualNodeService.load(TEST_CPB)
        val holdingIdentity = virtualNodeInfo.holdingIdentity
        val cpksMetadata = cpiInfoReadService.get(virtualNodeInfo.cpiIdentifier)!!.cpksMetadata.filter { it.isContractCpk() }
        val cpkFileChecksums = cpksMetadata.mapTo(HashSet()) { it.fileChecksum }
        ...
        // Send request to message processor (there is one redelivery left)
        val response = processor.onNext(
            state = createRedeliveryState(request, maxRedeliveries - 1),
            event = Record(TOPIC, REQUEST_ID, request)
        )
        ...
        assertThat(response.updatedState).isNotNull
        val resultState = response.updatedState
        assertThat(resultState!!.redeliveryNumber).isEqualTo(maxRedeliveries)
        assertThat(resultState.request).isEqualTo(request)
        assertThat(response.responseEvents).isEmpty()
    }

    @Test
    fun `returns error after final redelivery when CPK not available`() {
        val virtualNodeInfo = virtualNodeService.load(TEST_CPB)
        val holdingIdentity = virtualNodeInfo.holdingIdentity
        val cpksMetadata = cpiInfoReadService.get(virtualNodeInfo.cpiIdentifier)!!.cpksMetadata.filter { it.isContractCpk() }
        val cpkFileChecksums = cpksMetadata.mapTo(HashSet()) { it.fileChecksum }
        ...
        val response = processor.onNext(
            state = createRedeliveryState(request, maxRedeliveries),
            event = Record(TOPIC, REQUEST_ID, request)
        )
        ...
        assertThat(error.errorType).isEqualTo(ExternalEventResponseErrorType.PLATFORM)
        assertThat(error.exception.errorType).isEqualTo(NotReadyException::class.java.canonicalName)
        assertThat(externalEventResponse.requestId).isEqualTo(REQUEST_ID)
    }
    ...
    private fun createRedeliveryState(
        request: VerifyContractsRequest,
        redeliveryNumber: Int
