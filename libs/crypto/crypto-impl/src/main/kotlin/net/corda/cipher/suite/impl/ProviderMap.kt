...
class ProviderMap(
    ...
    private val cordaSecurityProvider = CordaSecurityProvider(keyEncoderProvider)
    ...
    private val cordaBouncyCastleProvider: Provider = SecurityProviderWrapper(BouncyCastleProvider().apply {
        addKeyInfoConverter(ID_CURVE_25519PH, object : AsymmetricKeyInfoConverter {
            override fun generatePublic(keyInfo: SubjectPublicKeyInfo) = decodePublicKey(EDDSA_ED25519, keyInfo.encoded)
            override fun generatePrivate(keyInfo: PrivateKeyInfo) = decodePrivateKey(EDDSA_ED25519, keyInfo.encoded)
        })
    }).apply {
