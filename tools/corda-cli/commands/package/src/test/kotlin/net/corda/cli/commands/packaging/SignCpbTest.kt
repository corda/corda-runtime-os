...
import net.corda.cli.commands.packaging.CreateCpbTest.Companion.CPB_SIGNER_NAME
import net.corda.cli.commands.packaging.TestSigningKeys.SIGNING_KEY_2_ALIAS
import net.corda.cli.commands.packaging.TestUtils.jarEntriesContentIsEqualInCpxs
import net.corda.cli.commands.packaging.TestUtils.jarEntriesExistInCpx
...
class SignCpbTest {
    ...
    fun `sign cpb with erasing signatures removes previous signatures and adds new with the same signature file name`() {
        ...
        val (createdManifestMainAttributes, createdManifestEntries) = TestUtils.getManifestMainAttributesAndEntries(
            createdCpb
        )
        val (signedManifestMainAttributes, signedManifestEntries) = TestUtils.getManifestMainAttributesAndEntries(
            signedCpb
        )
