...
class HttpRpcServerRequestsTest : HttpRpcServerTestBase() {
    ...
    fun `get invalid path returns 404 NOT FOUND`() {
        ...
        val invalidPathResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("invalidPath"), userName, password)
    ...
    fun `GET sanity returns Sane value`() {
        ...
        val sanityResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/sanity"), userName, password)
    ...
    fun `POST empty body doesn't throw exception if all parameters are optional`() {
        ...
        val pingResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest("health/ping", ""), userName, password)
        assertEquals(HttpStatus.SC_OK, pingResponse.responseStatus)
    ...
    fun `GET void returns nothing`() {
        val pingResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/void"), userName, password)
        assertEquals(HttpStatus.SC_OK, pingResponse.responseStatus)
    ...
    fun `GET plusone with not required list parameter not provided returns list`() {
        val plusOneResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/plusone"), List::class.java, userName, password)
        assertEquals(HttpStatus.SC_OK, plusOneResponse.responseStatus)
    ...
    fun `GET hello name returns string greeting name`() {
        ...
        val helloResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/hello/world?id=1"), userName, password)
    ...
    fun `GET hello2 name returns string greeting name`() {
        ...
        val helloResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/hello2/pathString?id=id"), userName, password)
        assertEquals(HttpStatus.SC_OK, helloResponse.responseStatus)
    ...
    fun `missing not required query parameter should not throw`() {
        val helloResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/hello2/pathString"), userName, password)
        assertEquals(HttpStatus.SC_OK, helloResponse.responseStatus)
    ...
    fun `POST plusone returns increased number`() {
        ...
        val reverseTextResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("health/plusone/2999999999"), userName, password)
    ...
    fun `POST negateinteger should return the negated number`() {
        ...
        val negateIntResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest("java/negateinteger", """{"number": 1}"""), userName, password)
    ...
    fun `POST negateprimitiveinteger should return the negated number`() {
        ...
        val negateIntResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest("java/negateprimitiveinteger", """{"number": 1}"""), userName, password)
        assertEquals(HttpStatus.SC_OK, negateIntResponse.responseStatus)
        assertEquals("application/json", negateIntResponse.headers["Content-Type"])
        assertEquals("-1", negateIntResponse.body)
    }
    ...
    fun `GET negate_long should return the negated number`() {
        ...
        val negateLongResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("java/negate_long?number=3000000000"), userName, password)
    ...
    fun `GET negate_boolean should return the negated boolean`() {
        ...
        val negateBooleanResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("java/negate_boolean?bool=true"), userName, password)
    ...
    fun `GET reverse text should return the reversed text`() {
        ...
        val reverseTextResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("java/reverse/txet"), userName, password)
        assertEquals(HttpStatus.SC_OK, reverseTextResponse.responseStatus)
    ...
    fun `GET without auth header returns HTTP 401 Unauthorized`() {
        val getPathResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<String>("health/sanity"))
    ...
    fun `GET invalid user returns HTTP 401 Unauthorized`() {
        val getPathResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/sanity"), "invalidUser", password)
        assertEquals(HttpStatus.SC_UNAUTHORIZED, getPathResponse.responseStatus)
    ...
    fun `GET invalid password returns HTTP 401 Unauthorized`() {
        val getPathResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/sanity"), userName, "invalidPassword")
        assertEquals(HttpStatus.SC_UNAUTHORIZED, getPathResponse.responseStatus)
        assertEquals("Error during user authentication", getPathResponse.body!!.asMapFromJson()["title"])
    }
    //TODO restore once proper users present
//    @Test
//    fun `GET valid user without permissions on requested method returns HTTP 403 Forbidden`() {
//        val getPathResponse = client.call(HttpVerb.GET, WebRequest<Any>("health/sanity"), userBob.username, password)
//        assertEquals(HttpStatus.SC_FORBIDDEN, getPathResponse.responseStatus)
//    }
    ...
    fun `GET valid user with valid permissions on requested get protocol version returns 200`() {
        val getPathResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/getprotocolversion"), userName, password)
        assertEquals(HttpStatus.SC_OK, getPathResponse.responseStatus)
    ...
    fun `GET invalid user without permissions on requested get protocol version returns 200`() {
        val getPathResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/getprotocolversion"), "invalid", "invalid")
        assertEquals(HttpStatus.SC_OK, getPathResponse.responseStatus)
        assertEquals("2", getPathResponse.body)
    }
    ...
    fun `POST body playground should not fail when values are passed`() {
        val reverseTextResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("health/bodyplayground", """ { "s1": "a", "s2": "b" } """), userName, password)
        assertEquals(HttpStatus.SC_OK, reverseTextResponse.responseStatus)
    ...
    fun `POST body playground should not fail when values are passed as null`() {
        val reverseTextResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("health/bodyplayground", """ { "s1": null, "s2": null } """), userName, password)
        assertEquals(HttpStatus.SC_OK, reverseTextResponse.responseStatus)
    ...
    fun `POST body playground should not fail when optional values are not passed`() {
        val reverseTextResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("health/bodyplayground", """ { "s1": null } """), userName, password)
        assertEquals(HttpStatus.SC_OK, reverseTextResponse.responseStatus)
        assertEquals(""""null null"""", reverseTextResponse.body)
    }
    ...
    fun `POST body playground should fail when required values are not passed`() {
        val reverseTextResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("health/bodyplayground", """ { "s2": null } """), userName, password)
        assertEquals(HttpStatus.SC_BAD_REQUEST, reverseTextResponse.responseStatus)
    }
    ...
    fun `POST timeCall should return correct time zone`() {
        ...
        val timeCallResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("health/timecall", """ { "time": { "time": "$time" } } """), userName, password)
    ...
    fun `Provided ZonedDateTime example can be parsed`() {
        val timeCallResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("health/timecall", """ { "time": { "time": "${ZonedDateTime::class.java.toExample()}" } } """), userName, password)
        assertEquals(HttpStatus.SC_OK, timeCallResponse.responseStatus)
    }
    ...
    fun `POST instantCall should parse and return correct Instant value`() {
        ...
        val instantCallResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("health/instantcall", """ { "instant": { "instant": "$instant" } } """), userName, password)
    ...
    fun `POST with custom marshalling should process the objects the custom way`() {
        val timeCallResponse = client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("customjson/printcustommarshal", """ { "s": "text" } """), userName, password)

        assertEquals(HttpStatus.SC_OK, timeCallResponse.responseStatus)
        assertEquals("{\"data\":\"custom text\"}", timeCallResponse.body)
    }

    //TODO restore this when having exceptions present

//    @Test
//    fun `StartFlowPermissionException should be converted to ForbiddenResponse`() {
//        val throwExceptionResponse = client.call(HttpVerb.GET, WebRequest<Any>("health/throwexception?exception=${StartFlowPermissionException::class.java.name}"), userName, password)
//        assertEquals(HttpStatus.SC_FORBIDDEN, throwExceptionResponse.responseStatus)
//    }
//
//    @Test
//    fun `FlowNotFoundException should be converted to NotFoundResponse`() {
//        val throwExceptionResponse = client.call(HttpVerb.GET, WebRequest<Any>("health/throwexception", queryParameters=mapOf("exception" to FlowNotFoundException::class.java.name)), userName, password)
//        assertEquals(HttpStatus.SC_NOT_FOUND, throwExceptionResponse.responseStatus)
//    }

    @Test
    fun `BadRequestException should be converted to BadRequestResponse`() {
        val throwExceptionResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/throwexception?exception=${BadRpcStartFlowRequestException::class.java.name}"), userName, password)
    ...
    fun `POST with non cordaSerializable should not run the init of the object`() {
        client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("customjson/unsafe", """ { "s": { "data": "value" } }"""), userName, password)
        client.call(net.corda.httprpc.tools.HttpVerb.POST, WebRequest<Any>("customjson/unsafe", """ { "s": { "unsafe": "value" } }"""), userName, password)
    ...
    fun `Unmapped exception should be converted to HttpResponseException with 500 status`() {
        ...
        val throwExceptionResponse = client.call(net.corda.httprpc.tools.HttpVerb.GET, WebRequest<Any>("health/throwexception?exception=java.lang.IllegalArgumentException"), userName, password)
