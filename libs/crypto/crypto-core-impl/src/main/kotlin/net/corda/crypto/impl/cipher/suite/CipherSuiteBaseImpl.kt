...
abstract class CipherSuiteBaseImpl : CipherSuiteBase {
    ...
    protected fun add(keyScheme: KeyScheme, encodingHandler: KeyEncodingHandler): Boolean =
        encodingHandlers.withWriteLock {
    ...
    protected fun add(keyScheme: KeyScheme, verifyHandler: VerifySignatureHandler): Boolean =
        verifyHandlers.withWriteLock {
            val existing = it[keyScheme.codeName]
    ...
    override fun getAllKeyEncodingHandlers(): List<KeyEncodingHandler> =
     ...
     override fun getAllSupportedKeySchemes(): List<KeySchemeInfo> = keySchemeMap.all()
 }