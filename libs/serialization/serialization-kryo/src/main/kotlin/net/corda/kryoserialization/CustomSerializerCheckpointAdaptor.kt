 ...
 */
internal class CustomSerializerCheckpointAdaptor<OBJ, PROXY>
    (private val userSerializer: CheckpointCustomSerializer<OBJ, PROXY>) : Serializer<OBJ>() {
    ...
    /**
     * Throws a `CustomCheckpointSerializersHaveChangedException` if the serializer type in the kryo stream does not
     * match the serializer type for this custom serializer.
     *
    ...
    private fun checkSerializerType(checkpointSerializerType: String) {
        if (checkpointSerializerType != serializerName)
            throw CustomCheckpointSerializersHaveChangedException(
                "The custom checkpoint serializers have changed " +
                        "while checkpoints exist. Please restore the CorDapps to when this checkpoint was created."
            )
    }
}
 ...
 *
 * This was expected to happen if the user adds or removes CorDapps while checkpoints exist but it turned out that
 * registering serializers as default made the system reliable.
 */
