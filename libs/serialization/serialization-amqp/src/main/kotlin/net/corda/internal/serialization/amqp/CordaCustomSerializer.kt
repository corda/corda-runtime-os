 ...
 */
private const val CORDA_TYPE = 0
...
/**
 * Wrapper class for internal Corda serializers
 *
 * OSGi will provide us with a list of classes that implement [InternalCustomSerializer].
 * This class takes an instance of one of those objects and embeds it within a serialization
 * context associated with a serializer factory by creating an instance of this class and
 * registering that with a [SerializerFactory]
 *
 ...
 * for
 */
class CordaCustomSerializer(
    private val serializer: InternalCustomSerializer<*, *>,
    ...
    init {
        ...
        // We require that each internal custom serializer implements
        // InternalCustomSerializer directly.
        val types = serializer::class.java.genericInterfaces
            .filterIsInstance<ParameterizedType>()
            .filter { it.rawType === InternalCustomSerializer::class.java }
            .flatMap { it.actualTypeArguments.asList() }

        if (types.size != 2) {
            throw AMQPNotSerializableException(
                CordaCustomSerializer::class.java,
                "Unable to determine serializer parent types")
        }

        type = types[CORDA_TYPE]
    ...
    override fun writeObject(
        ...
        val proxy = (serializer as InternalCustomSerializer<Any, Any?>).toProxy(obj, context)
    ...
    override fun readObject(
        ...
        @Suppress("unchecked_cast")
        return (serializer as InternalCustomSerializer<Any, Any>).fromProxy(proxy, context)
    }

    /**
     * For internal Corda plugin serializers we are going to insist on exact type matching,
