...
import com.r3.corda.demo.interop.tokens.states.DehydratedTokenState
...
private class TransactionInfo(
    val inputStates: List<DehydratedTokenState>,
    val outputStates: List<DehydratedTokenState>
)
...
class GetTransactionInfoFlow : ClientStartableFlow {
    ...
    private fun getTransactionInfo(transaction: UtxoLedgerTransaction): GetTransactionInfoFlowResult {
        val inputStates = try {
            transaction.getInputStateAndRefs(TokenState::class.java).map { it.state.contractState.dehydrate() }
        } catch (e: Exception) {
        ...
        val outputStates = try {
            transaction.getOutputStateAndRefs(TokenState::class.java).map { it.state.contractState.dehydrate() }
        } catch (e: Exception) {
            return GetTransactionInfoFlowResult(
