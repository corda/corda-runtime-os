...
internal object DataSourceFactoryHelper {
    private val log = LoggerFactory.getLogger(DataSourceFactoryHelper::class.java)
    ...
    fun DataSourceFactory.createFromConfig(config: SmartConfig): CloseableDataSource {
        ...
        log.debug("Given configuration: ${config.toSafeConfig().root().render(ConfigRenderOptions.concise())}")
        log.debug("Fallback configuration: ${dbFallbackConfig.root().render(ConfigRenderOptions.concise())}")
        ...
        log.debug("Creating DB connection for: $driver, $jdbcUrl, $username, $maxPoolSize")
        return this.create(
            driverClass = driver,
            jdbcUrl = jdbcUrl,
            username = username,
