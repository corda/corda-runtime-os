...
class BaseFlowFactoryTest {
    ...
    fun `should be able to construct an initiating flow from a class string`() {
        val flow = BaseFlowFactory().createInitiatingFlow(x500, PingAckFlow::class.java.name)
        assertThat(flow, `isA`(PingAckFlow::class.java))
    }

    @Test
    fun `should be able to construct a responder flow from its class`() {
        val flow = BaseFlowFactory().createResponderFlow(x500, PingAckResponderFlow::class.java)
    ...
    fun `should error if the flow class is not recognized`() {
        ...
        assertThrows<UnrecognizedFlowClassException> { BaseFlowFactory().createInitiatingFlow(x500, notAFlowClass.name)}
    }
    ...
    fun `should error if initiating flow being constructed is not an RPCStartableFlow`() {
        ...
        assertThrows<UnrecognizedFlowClassException> { BaseFlowFactory().createInitiatingFlow(x500, notAnInitFlow.name)}
    }

    @Test
    fun `should error if responder flow is not a ResponderFlow`() {
        val flowClass = ValidStartingFlow::class.java
        assertThrows<UnrecognizedFlowClassException> { BaseFlowFactory().createResponderFlow(x500, flowClass) }
    }

    @Test
    fun `should error if the flow cannot be constructed`() {
        val flowClass = NonConstructableFlow::class.java
        assertThrows<NoDefaultConstructorException> { BaseFlowFactory().createInitiatingFlow(x500, flowClass.name) }
    }
}