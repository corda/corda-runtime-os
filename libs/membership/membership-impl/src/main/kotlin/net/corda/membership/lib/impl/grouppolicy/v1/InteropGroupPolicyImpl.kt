...
class InteropGroupPolicyImpl(
    ...
    internal inner class InteropP2PParametersSessionsImpl(rootNode: JsonNode) : GroupPolicy.P2PParameters {
        ...
        override val sessionPki = GroupPolicyConstants.PolicyValues.P2PParameters.SessionPkiMode.STANDARD

        override val sessionTrustRoots: Collection<String>? =
            p2pParameters.getMandatoryStringList(rootNode.get("sessionTrustRoots").asText())
                .apply {
                    if (isEmpty()) {
                        throw BadGroupPolicyException(getMissingCertError(rootNode.get("sessionTrustRoots").asText()))
                    }
                }


        override val tlsTrustRoots: Collection<String> =
            p2pParameters.getMandatoryStringList(rootNode.get("tlsTrustRoots").asText()).apply {
                if (isEmpty()) {
                    throw BadGroupPolicyException(getMissingCertError(rootNode.get("tlsTrustRoots").asText()))
                }
            }.onEachIndexed { index, pemCert ->
                validatePemCert(
                    pemCert,
                    GroupPolicyConstants.PolicyKeys.P2PParameters.TLS_TRUST_ROOTS,
                    index
                )
            }
