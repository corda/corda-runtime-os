...
class UtxoLedgerGroupParametersPersistenceServiceImplTest {
    ...
    fun `persist group parameters that are not cached adds them to the cache`() {
        ...
        whenever(signedGroupParameters.bytes).thenReturn(byteArrayOf(1))
        whenever(signedGroupParameters.signature).thenReturn(DigitalSignatureWithKey(mock(), byteArrayOf(2)))
        whenever(signedGroupParameters.signatureSpec).thenReturn(mock())
        whenever(groupParametersCache.get(any())).thenReturn(null)
