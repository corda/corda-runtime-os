...
class PlatformCipherSuiteMetadataImpl : PlatformCipherSuiteMetadata {
    companion object {
    ...
    override val secureRandom: SecureRandom = SecureRandom.getInstance(
    ...
    override val supportedSigningSchemes: Map<KeyScheme, KeySchemeInfo> = mapOf(
        RSA.scheme to RSA,
    ...
    override fun findKeyFactory(algorithm: AlgorithmIdentifier): KeyFactory {
        val scheme = findKeyScheme(algorithm)
        return keyFactories[scheme]
    }

    override fun providerFor(scheme: KeyScheme): Provider = schemeProviderMap.getValue(scheme)

    override fun providerForDigest(algorithmName: String): Provider = providers.getValue(
    ...
    override fun decode(encodedKey: ByteArray): PublicKey? = try {
        ...
        val keyFactory = findKeyFactory(subjectPublicKeyInfo.algorithm)
