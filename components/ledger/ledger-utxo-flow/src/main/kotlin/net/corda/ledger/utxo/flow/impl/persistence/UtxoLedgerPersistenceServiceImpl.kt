...
class UtxoLedgerPersistenceServiceImpl @Activate constructor(
    ...
    companion object {
        val log = LoggerFactory.getLogger(UtxoLedgerPersistenceServiceImpl::class.java)
    }
    ...
    @Suspendable
    override fun persistMerkleProofIfDoesNotExist(transactionId: SecureHash, groupId: Int, merkleProof: MerkleProof) {
        recordSuspendable({ ledgerPersistenceFlowTimer(UpdateTransactionStatus) }) @Suspendable { // TODO Fix metric
            wrapWithPersistenceException {
                externalEventExecutor.execute(
                    PersistMerkleProofIfDoesNotExistExternalEventFactory::class.java,
                    PersistMerkleProofIfDoesNotExistParameters(transactionId.toString(), groupId, merkleProof)
                )
            }
        }
    }
    ...
    override fun findMerkleProofs(transactionId: SecureHash, groupId: Int): List<MerkleProof> {
        return recordSuspendable({ ledgerPersistenceFlowTimer(UpdateTransactionStatus) }) @Suspendable { // TODO Fix metric
            wrapWithPersistenceException {
                externalEventExecutor.execute(
                    FindMerkleProofsExternalEventFactory::class.java,
                    FindMerkleProofsParameters(transactionId.toString(), groupId)
                )
            }
        }.firstOrNull()?.let {
            serializationService.deserialize<List<MerkleProofDto>>(it.array())
                .map { merkleProofDto ->
                    log.info("[MerkleProofPoC] Found & returned: $merkleProofDto")
                    val mp = merkleProofFactory.createMerkleProof(
                        merkleProofDto.merkleProofDtoId.transactionId,
                        merkleProofDto.merkleProofDtoId.groupId,
                        merkleProofDto.treeSize,
                        merkleProofDto.leavesWithData,
                        merkleProofDto.hashes
                    )
                    log.info("[MerkleProofPoC] created merkleproof: $mp")
                    mp
                }
        } ?: emptyList()
    }

    override fun persistFilteredTransaction(filteredTransaction: UtxoFilteredTransaction) {
        recordSuspendable({ ledgerPersistenceFlowTimer(UpdateTransactionStatus) }) @Suspendable { // TODO Fix metric
            wrapWithPersistenceException {
                externalEventExecutor.execute(
