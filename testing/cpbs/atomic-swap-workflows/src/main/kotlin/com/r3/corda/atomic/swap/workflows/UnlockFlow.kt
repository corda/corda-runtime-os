...
class UnlockFlow : ClientStartableFlow {
    ...
    override fun call(requestBody: ClientRequestBody): String {
        ...
        try {
            ...
            val unconsumedLockStates = ledgerService.findUnconsumedStatesByExactType(LockState::class.java, 10, Instant.now())
            val unconsumedLockStatesWithId = unconsumedLockStates.results.filter { it.state.contractState.assetId == stateId }
            ...
            val unconsumedAssetStates = ledgerService.findUnconsumedStatesByExactType(Asset::class.java, 10, Instant.now())
            val unconsumedAssetStatesWithId = unconsumedAssetStates.results.filter { it.state.contractState.assetId == stateId }
