...
data class HttpRpcClientConfig internal constructor(
    ...
    fun username(username: String) = copy(authenticationConfig = if (authenticationConfig is BasicAuthenticationConfig)
        authenticationConfig.copy(username = username) else BasicAuthenticationConfig(username))
    fun password(password: String) = copy(authenticationConfig = if (authenticationConfig is BasicAuthenticationConfig)
