 ...
 *
 * It has a main use: Comparison with [LocalTypeInformation] to determine compatibility and whether type evolution
 * should be attempted.
 *
 * A [TypeLoader] knows how to load types described by [RemoteTypeInformation], so that every piece of
 * [RemoteTypeInformation] is matched to a corresponding local [Type] for which [LocalTypeInformation] can be generated.
 * Once we have both [RemoteTypeInformation] and [LocalTypeInformation] in hand, we can make decisions about the
 * compatibility between the remote and local type schemas.
 *
