...
open class FreshKeySigningServiceImpl(
    ...
    init {
        ...
        if (defaultFreshKeySignatureScheme.codeName !in freshKeyMap) {
            throw CryptoServiceException("The default signature schema '${defaultFreshKeySignatureScheme.codeName}' is not supported, " +
            ...
            try {
                ...
                val signedBytes = if (keyData.alias != null) {
                    ...
                    if (keyData.privateKeyMaterial == null || keyData.masterKeyAlias.isNullOrBlank()) {
                        throw IllegalArgumentException("Cannot perform the sign operation as either the key material is absent or the master key alias.")
                    }
