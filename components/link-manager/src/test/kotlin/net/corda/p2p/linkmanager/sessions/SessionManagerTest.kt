...
class SessionManagerTest {
    ...
    companion object {
        ...
        val OUR_MEMBER_INFO = LinkManagerNetworkMap.MemberInfo(
            OUR_PARTY, OUR_KEY.public, KeyAlgorithm.ECDSA,
        ...
        val PEER_MEMBER_INFO = LinkManagerNetworkMap.MemberInfo(
            PEER_PARTY, PEER_KEY.public, KeyAlgorithm.ECDSA,
    ...
    inner class NoHeartbeatTests {
        ...
        fun `when no session exists, if source member info is missing from network map no message is sent`() {
            ...
            verify(sessionReplayer, never()).addMessageForReplay(any(), any())
            loggingInterceptor.assertSingleWarning(
                "Attempted to start session negotiation with peer $PEER_PARTY " +
        ...
        fun `when an initiator hello is received, a responder hello is returned`() {
            ...
            val initiatorHelloMsg = InitiatorHelloMessage(
                header, ByteBuffer.wrap(PEER_KEY.public.encoded),
        ...
        fun `when an initiator hello is received, but peer's member info is missing from network map, then message is dropped`() {
            ...
            loggingInterceptor.assertSingleWarning(
                "Received ${InitiatorHelloMessage::class.java.simpleName} with sessionId ${sessionId}. " +
        ...
        fun `when responder hello is received without an existing session, the message is dropped`() {
            ...
            val responderHello =
                ResponderHelloMessage(header, ByteBuffer.wrap(PEER_KEY.public.encoded), ProtocolMode.AUTHENTICATED_ENCRYPTION)
            val responseMessage = sessionManager.processSessionMessage(LinkInMessage(responderHello))

            assertThat(responseMessage).isNull()
            loggingInterceptor.assertSingleWarning(
                "Received ${ResponderHelloMessage::class.java.simpleName} with sessionId $sessionId " +
        ...
        fun `when responder hello is received, but private key cannot be found to sign, message is dropped`() {
            ...
            loggingInterceptor.assertSingleWarningContains(
                "The ${ResponderHelloMessage::class.java.simpleName} with sessionId " +
        ...
        fun `when initiator handshake is received, a responder handshake is returned and session is established`() {
            ...
            assertThat(
                sessionManager.getSessionById(sessionId)).isInstanceOfSatisfying(SessionManager.SessionDirection.Inbound::class.java
            ) {
                assertThat(it.session).isEqualTo(session)
            }
        }
        ...
        fun `when applyNewConfiguration, after the inbound session is established, the session is removed`() {
            ...
            verify(publisherWithDominoLogic.constructed()[1])
                .publish(listOf(Record(Schema.SESSION_OUT_PARTITIONS, sessionId, null)))
    ...
    inner class HeartbeatTests {
        ...
        private lateinit var configHandler: SessionManagerImpl.SessionManagerConfigChangeHandler
        private lateinit var heartbeatConfigHandler: SessionManagerImpl.HeartbeatManager.HeartbeatManagerConfigChangeHandler
        private var createResourcesCallbacks = mutableMapOf<String, Set<((resources: ResourcesHolder) -> CompletableFuture<Unit>)>>()
        private val dominoTile = Mockito.mockConstruction(DominoTile::class.java) { mock, context ->
            @Suppress("UNCHECKED_CAST")
            whenever(mock.withLifecycleLock(any<() -> Any>())).doAnswer { (it.arguments.first() as () -> Any).invoke() }
            @Suppress("UNCHECKED_CAST")
            ...
            if (context.arguments()[2] != null) {
                @Suppress("UNCHECKED_CAST")
                createResourcesCallbacks.compute(context.arguments()[0] as String) { _, it ->
                    return@compute if (it == null) {
                        mutableSetOf(context.arguments()[2] as ((resources: ResourcesHolder) -> CompletableFuture<Unit>))
                    } else {
                        it + context.arguments()[2] as ((resources: ResourcesHolder) -> CompletableFuture<Unit>)
                    }
                }
            }
        }
        ...
        fun `when responder hello is received, the session is pending, if no response is received, the session times out`() {
            val sessionManager = SessionManagerImpl(
                ...
                setRunning()
                createResourcesCallbacks[PublisherWithDominoLogic::class.java.simpleName]?.forEach { it(mock()) }
                configHandler.applyNewConfiguration(
                    SessionManagerImpl.SessionManagerConfig(MAX_MESSAGE_SIZE, setOf(ProtocolMode.AUTHENTICATION_ONLY)),
                    null,
                    mock(),
                ...
                createResourcesCallbacks[SessionManagerImpl.HeartbeatManager::class.java.simpleName]?.forEach { it(mock()) }
            }
            sessionManager.start()
        ...
        fun `when a data message is sent, heartbeats are sent, if these are not acknowledged the session times out`() {
            ...
            val tombstoneRecords = mutableListOf<Record<*, *>>()
            fun callback(records: List<Record<*, *>>) {
                ...
                } else if (record.topic == Schema.SESSION_OUT_PARTITIONS) {
            ...
            assertThat(tombstoneRecords).isEqualTo(listOf(Record(Schema.SESSION_OUT_PARTITIONS, sessionState.sessionId, null)))
