    ...
    companion object {
        fun make(typeInformation: LocalTypeInformation, factory: LocalSerializerFactory): ObjectSerializer {
            ...
            return when (typeInformation) {
                is LocalTypeInformation.Composable -> makeForComposable(
                    typeInformation,
                    typeNotation,
                    typeDescriptor,
                    factory
                ...
                is LocalTypeInformation.AnInterface, is LocalTypeInformation.Abstract -> makeForAbstract(
                    typeNotation,
                    typeInformation,
                    typeDescriptor,
                    factory
        ...
        private fun makeForAbstract(
            ...
            val writer =
                ComposableObjectWriter(typeNotation, typeInformation.interfacesOrEmptyList, propertySerializers)
        ...
        private fun makePropertySerializers(
            ...
            factory: LocalSerializerFactory
        ): Map<String, PropertySerializer> = properties.mapValues { (name, property) ->
...
class ComposableObjectSerializer(
    ...
    private val writer: ComposableObjectWriter
) : ObjectSerializer {
    ...
    override fun readObject(
        ...
        context: SerializationContext
    ): Any = reader.readObject(obj, serializationSchemas, metadata, input, context)
}
...
class ComposableObjectReader(
    ...
    fun readObject(
        ...
        input: DeserializationInput,
        context: SerializationContext
    ): Any = ifThrowsAppend({ typeIdentifier.prettyPrint(false) }) {
        if (obj !is List<*>) throw NotSerializableException("Body of described type is unexpected $obj")
        if (obj.size < propertySerializers.size) {
            throw NotSerializableException(
                "${obj.size} objects to deserialize, but " + "${propertySerializers.size} properties in described type ${
...
class AbstractObjectSerializer(
    ...
    override fun readObject(
        ...
        input: DeserializationInput,
        context: SerializationContext
    ): Any = throw UnsupportedOperationException("Cannot deserialize abstract type ${type.typeName}")
}
    ...
    companion object {
        fun make(
            ...
            val propertySerializers =
                makePropertySerializers(properties, remoteTypeInformation.properties, sandboxGroup)
            val reader = ComposableObjectReader(
        ...
        private fun makePropertySerializers(
            ...
            sandboxGroup: SandboxGroup
        ): Map<String, PropertySerializer> = remoteProperties.mapValues { (name, property) ->
            ...
            ComposableTypePropertySerializer.makeForEvolution(
                name,
                isCalculated,
                property.type.typeIdentifier,
                type
            )
        }
    }
