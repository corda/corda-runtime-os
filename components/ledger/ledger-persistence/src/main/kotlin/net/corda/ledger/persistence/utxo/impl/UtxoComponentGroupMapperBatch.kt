 ...
 */
class UtxoComponentGroupMapperBatch(private val transactionId: String) : ComponentGroupMapper {
    override fun map(tuples: List<Tuple>): Map<Int, List<ByteArray>> {
        ...
        tuples.forEach { columns ->
            val groupIdx = (columns[1] as Number).toInt()
            val leafIdx = (columns[2] as Number).toInt()
            val data = columns[3] as ByteArray
