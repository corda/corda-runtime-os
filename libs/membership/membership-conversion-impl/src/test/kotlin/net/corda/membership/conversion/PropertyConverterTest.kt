...
import net.corda.membership.testkit.DummyNumber
...
class PropertyConverterTest {
    companion object {
        private val propertyConverter = PropertyConverterImpl(emptyList())
    ...
    @Test
    fun `retrieving custom converters should work`() {
        assertEquals(emptyList(), propertyConverter.customConverters)
    }
    ...
    fun `converter should throw IllegalStateException when there is no converter for the expected type`() {
        ...
        assertEquals("Unknown '${DummyNumber::class.java.name}' type.", ex.message)
    }
