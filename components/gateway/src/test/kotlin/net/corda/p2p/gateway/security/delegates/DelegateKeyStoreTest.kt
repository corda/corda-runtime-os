...
class DelegateKeyStoreTest {
    val onePublicKey = mock<PublicKey> {
    ...
    fun `engineLoad ignores unknown types`() {
        val keyStore = DelegateKeyStore(mutableMapOf())
    ...
    @Test
    fun `engineGetCertificate set the service correctly`() {
        val serviceMap = mutableMapOf<PublicKey, SigningService?>()
        val keyStore = DelegateKeyStore(serviceMap)
        keyStore.engineLoad(DelegateKeyStore.LoadParameter(service))

        keyStore.engineGetCertificate("one")

        assertThat(serviceMap[onePublicKey]).isEqualTo(service)
    }
    ...
    inner class UnsupportedOperationTests {
        private val keyStore = DelegateKeyStore(mutableMapOf())
        @Test
