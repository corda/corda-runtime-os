 ...
 * appropriate when creating instances of or extending [HttpApiException].
 *
 * This enum will define all HTTP status codes and their causes. They also include a reason code which will help to identify particular error responses and aid in debugging and support issues.
 *
 ...
 * 5XX - indicate a problem occurred on the server side while processing the request. An application can't perform any action to correct a 500-level error.
 *
 * @param statusCode the http status code for the http response.
 * @param reasonCode unique identifier for a response code.
 */
enum class ResponseCode constructor(val statusCode: Int, val reasonCode: String) {
    /**
     * Signals the exception occurred due to invalid input data in the request or from a resource identified by the request.
     */
    BAD_REQUEST(400, "4001"),

    /**
     * Signals the user authentication failed.
     */
    NOT_AUTHENTICATED(401, "4002"),
     ...
     */
    RESOURCE_NOT_FOUND(404, "4004"),

    /**
     * An error occurred internally.
     */
    INTERNAL_SERVER_ERROR(500, "5001"),
     ...
     */
    UNEXPECTED_ERROR(500, "5002"),
    ;

    override fun toString(): String {
        return "$name(statusCode=$statusCode, reasonCode='$reasonCode')"
    }
}