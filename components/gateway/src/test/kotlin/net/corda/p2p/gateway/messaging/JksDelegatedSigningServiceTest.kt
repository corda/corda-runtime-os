...
class JksDelegatedSigningServiceTest {
    ...
    private val ecSignatureServices = DelegatedSigningService.Hash.values().flatMap { hash ->
        listOf(
    ...
    inner class AliasTests {
        ...
        fun `alias return the correct names for RSA certificate`() {
            val service = JksDelegatedSigningService("", ByteArray(0), "password")

            assertThat(service.aliases.map { it.name }).contains(rsaCertificateAlias)
        }

        @Test
        fun `alias return the correct names for EC certificate`() {
        ...
        @Test
        fun `alias return the correct certificates for EC certificate`() {
            val service = JksDelegatedSigningService("name", ByteArray(0), "password")
        ...
        fun `aliases load with the correct data`() {
            ...
            val service = JksDelegatedSigningService("name", "hello".toByteArray(), "password")
    ...
    inner class SignTests {
        ...
        inner class EcSignTests {
            ...
            fun `ec sign return the signature`() {
                val data = "data".toByteArray()
                val hash = DelegatedSigningService.Hash.SHA256
                val service = JksDelegatedSigningService("name", "hello".toByteArray(), "password")
                ...
                val sign = mockStatic(Signature::class.java).use { mockSignature ->
                    mockSignature.`when`<Signature> {
                        Signature.getInstance(
                            DelegatedSigningService.Hash.SHA256.ecName,
                            ecSignatureProvider
                        )
                    }.doReturn(
                        signature
            ...
            fun `ec sign sends the correct data`() {
                ...
                mockStatic(Signature::class.java).use { mockSignature ->
                    mockSignature.`when`<Signature> {
                        Signature.getInstance(
                            DelegatedSigningService.Hash.SHA512.ecName,
                            ecSignatureProvider
                        )
                    }.doReturn(
                        signature
