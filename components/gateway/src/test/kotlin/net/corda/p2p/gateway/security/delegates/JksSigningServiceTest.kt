...
class JksSigningServiceTest {
    ...
    inner class VerifyTests {
        @Test
        fun `unknown algorithm verify throw exception`() {
            val publicKey = mock<PublicKey> {
                on { algorithm } doReturn "Unknown"
            }
            val data = "data".toByteArray()
            val sign = "signature".toByteArray()
            val hash = SigningService.Hash.SHA384
            val service = JksSigningService("hello".toByteArray(), "password")

            assertThrows<SecurityException> {
                service.verify(publicKey, hash, data, sign)
            }
        }

        @Nested
        inner class RsaVerifyTests {
            @Test
            fun `rsa verify return the correct data`() {
                val publicKey = mock<PublicKey> {
                    on { algorithm } doReturn "RSA"
                }
                val data = "data".toByteArray()
                ...
                val service = JksSigningService("hello".toByteArray(), "password")
                val signature = mock<Signature> {
                    on { verify(sign) } doReturn true
                }
                val verified = mockStatic(Signature::class.java).use { mockSignature ->
                    mockSignature.`when`<Signature> {
                        Signature.getInstance(
                            RSA_SINGING_ALGORITHM,
                            rsaSignatureProvider
            ...
            @Test
            fun `rsa verify sends the correct data`() {
                val publicKey = mock<PublicKey> {
                    on { algorithm } doReturn "RSA"
                }
                val data = "data".toByteArray()
                ...
                verify(signature).initVerify(publicKey)
                verify(signature).setParameter(hash.rsaParameter)
                verify(signature).update(data)
        ...
        @Nested
        inner class EcVerifyTests {
            @Test
            fun `ec verify return the correct data`() {
                val publicKey = mock<PublicKey> {
                    on { algorithm } doReturn "EC"
                }
                val data = "data".toByteArray()
            ...
            @Test
            fun `ec verify sends the correct data`() {
                val publicKey = mock<PublicKey> {
                    on { algorithm } doReturn "EC"
                }
                val data = "data".toByteArray()
