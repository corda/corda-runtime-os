...
public class UpdateBilateralFlow implements ClientStartableFlow {

    private final static Logger log = LoggerFactory.getLogger(UpdateBilateralFlow.class);
    ...
    public String call(ClientRequestBody requestBody) {
        ...
        log.info("UpdateBilateralFlow.call() called");
        ...
        try {
            ...
            if (bilateralStateAndRefsWithId.size() != 1) throw new CordaRuntimeException("Multiple or zero Chat states with id " + bStateId + " found");
            StateAndRef<BilateralState> inputbilateralStateStateAndRef = bilateralStateAndRefsWithId.get(0);
            BilateralState input = inputbilateralStateStateAndRef.getState().getContractState();

            // Get MemberInfos for the Vnode running the flow and the otherMember.
            MemberInfo dtcc = memberLookup.myInfo();//DTCC is the bilateral initiator
            MemberInfo deliver = requireNonNull(memberLookup.lookup(input.getDeliver()),"MemberLookup can't find otherMember specified in flow arguments.");
            MemberInfo receiver = requireNonNull(memberLookup.lookup(input.getReceiver()),"MemberLookup can't find otherMember specified in flow arguments.");
            MemberInfo dtccObserver = requireNonNull(memberLookup.lookup(input.getDtccObserver()),"MemberLookup can't find otherMember specified in flow arguments.");

            // Create the BilateralState from the input arguments and member information.
            BilateralState bilateralState = input.update(flowArgs.getTradeStatus(), flowArgs.getShrQty(), flowArgs.getSecurityID(),
          ...
          /*
            ...
            */
            ...
            UtxoTransactionBuilder txBuilder = ledgerService.createTransactionBuilder()
                    ...
                    .addCommand(new BilateralContract.Update())
                    .addSignatories(bilateralState.getParticipants());
...
{
    "clientRequestId": "updateBilateral-1",
    "flowClassName": "com.r3.developers.csdetemplate.ION.UpdateBilateralFlow",
    "requestData": {
