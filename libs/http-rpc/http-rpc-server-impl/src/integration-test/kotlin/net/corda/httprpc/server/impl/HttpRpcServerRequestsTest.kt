...
class HttpRpcServerRequestsTest : HttpRpcServerTestBase() {
    companion object {
        ...
        fun setUpBeforeClass() {
            ...
            server = HttpRpcServerImpl(
                listOf(TestHealthCheckControllerImpl(), TestJavaPrimitivesControllerImpl(), CustomSerializationControllerImpl()),
                securityManager,
                httpRpcSettings,
                true
            ).apply { start() }
    ...
    fun `POST negateprimitiveinteger should return the negated number`() {
        ...
        val negateIntResponse =
            client.call(HttpVerb.POST, WebRequest("java/negateprimitiveinteger", """{"number": 1}"""), userName, password)
        assertEquals(HttpStatus.SC_OK, negateIntResponse.responseStatus)
        assertEquals("application/json", negateIntResponse.headers["Content-Type"])
        assertEquals("-1", negateIntResponse.body)
    }
...
//    }
    ...
    fun `POST body playground should not fail when values are passed`() {
        val reverseTextResponse =
            client.call(HttpVerb.POST, WebRequest<Any>("health/bodyplayground", """ { "s1": "a", "s2": "b" } """), userName, password)
        assertEquals(HttpStatus.SC_OK, reverseTextResponse.responseStatus)
    ...
    fun `POST body playground should not fail when values are passed as null`() {
        val reverseTextResponse =
            client.call(HttpVerb.POST, WebRequest<Any>("health/bodyplayground", """ { "s1": null, "s2": null } """), userName, password)
        assertEquals(HttpStatus.SC_OK, reverseTextResponse.responseStatus)
...
//            client.call(HttpVerb.POST, WebRequest<Any>("health/bodyplayground", """ { "s1": null } """), userName, password)
    ...
    fun `POST with custom marshalling should process the objects the custom way`() {
        val timeCallResponse =
            client.call(HttpVerb.POST, WebRequest<Any>("customjson/printcustommarshal", """{ "s": "text" }"""), userName, password)
...
//    }
    ...
    fun `BadRequestException should be converted to BadRequestResponse`() {
        val throwExceptionResponse = client.call(
            HttpVerb.GET,
            WebRequest<Any>("health/throwexception?exception=${BadRpcStartFlowRequestException::class.java.name}"),
            userName,
            password
        )
    ...
    // I disagree with the [CordaSerializable] requirements
    @Disabled
    @Test
    ...
    fun `Unmapped exception should be converted to HttpResponseException with 500 status`() {
        ...
        val throwExceptionResponse = client.call(
            HttpVerb.GET,
            WebRequest<Any>("health/throwexception?exception=java.lang.IllegalArgumentException"),
            userName,
            password
        )
