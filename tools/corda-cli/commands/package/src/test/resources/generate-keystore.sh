...
keytool -genkeypair -alias "${SIGNING_KEY_1}" -dname "cn=CPI Command Example - Signing Key 1, o=R3, L=London, c=GB" \
        -keyalg ${KEY_ALG} -validity 4000 -keystore signingkeys.pfx -storetype pkcs12 -storepass "${STOREPASS}"

keytool -genkeypair -alias "${SIGNING_KEY_2}" -dname "cn=CPI Command Example - Signing Key 2, o=R3, L=London, c=GB" \
        -keyalg ${KEY_ALG} -validity 4000 -keystore signingkeys.pfx -storetype pkcs12 -storepass "${STOREPASS}" \
