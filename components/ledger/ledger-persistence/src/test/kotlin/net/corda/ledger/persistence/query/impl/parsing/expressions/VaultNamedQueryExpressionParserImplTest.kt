...
class VaultNamedQueryExpressionParserImplTest {
    ...
    fun `field name not in quotes is parsed as PathReference`() {
        ...
        assertMatches(expression, listOf(
            PathReference("these"),
            PathReference("ARE"),
            Number("123"),
            PathReference("field"),
    ...
    fun `json cast to is parsed as JsonCast`() {
        assertMatches(expressionParser.parse("a::int::int::int"),
            listOf(JsonCast(
                listOf(PathReference("a")),
        ...
        assertMatches(expressionParser.parse("b::date"),
            listOf(JsonCast(
                listOf(PathReference("b")),
                listOf(SqlType("date"))
            ))
