...
class BaseFlowFactory : FlowFactory {
    ...
    override fun createInitiatingFlow(member: MemberX500Name, flowClassName: String): ClientStartableFlow {
        val flowClass = Class.forName(flowClassName)
            ?: throw FlowClassNotFoundException(flowClassName)

        if (ClientStartableFlow::class.java.isAssignableFrom(flowClass)) {
            return createFlow(flowClass)
        } else {
            throw UnrecognizedFlowClassException(flowClass, listOf(ClientStartableFlow::class.java))
        }
