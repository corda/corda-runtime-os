...
data class SerializationContextImpl @JvmOverloads constructor(
    ...
    override fun withClassInfoService(classInfoService: Any): SerializationContext =
        copy(classInfoService = classInfoService)
    ...
    override fun withEncodingWhitelist(encodingWhitelist: EncodingWhitelist) =
        copy(encodingWhitelist = encodingWhitelist)
}

open class SerializationFactoryImpl(
    // TODOs: This is read-mostly. Probably a faster implementation to be found.
    ...
    private fun schemeFor(
        ...
        return (schemes[lookupKey] ?: schemes.computeIfAbsent(lookupKey) {
            registeredSchemes.filter { it.canDeserializeVersion(magic, target) }
                .forEach { return@computeIfAbsent it } // XXX: Not single?
            logger.warn(
    ...
    override fun <T : Any> deserialize(byteSequence: ByteSequence, clazz: Class<T>, context: SerializationContext): T {
        return asCurrent {
            withCurrentContext(context) {
                schemeFor(byteSequence, context.useCase).first.deserialize(
                    byteSequence,
                    clazz,
                    context
                )
    ...
    override fun <T : Any> serialize(obj: T, context: SerializationContext): SerializedBytes<T> {
        return asCurrent {
            withCurrentContext(context) {
                schemeFor(
                    context.preferredSerializationVersion,
                    context.useCase
                ).first.serialize(obj, context)
            }
        }
    }
