...
class InteropProcessor(
    ...
    private fun processOutboundEvent(state: InteropState?, sessionEvent: SessionEvent):
        ...
        val translatedSource = sourceIdentity.apply {
            val interopIdentity = checkNotNull(sourceRegistryView.getOwnedIdentities()[UUID.fromString(interopGroupId)]) {
        ...
        val translatedDestination = destinationIdentity.apply {
            val groupIdentities = checkNotNull(sourceRegistryView.getIdentitiesByGroupId()[UUID.fromString(interopGroupId)]) {
