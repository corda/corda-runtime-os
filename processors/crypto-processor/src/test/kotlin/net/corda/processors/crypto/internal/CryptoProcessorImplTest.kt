...
class CryptoProcessorImplTest {
    ...
    fun `bus subscriptions created on ConfigChangedEvent, closed and re-created on new ConfigChangedEvent`() {
        ...
        val hsmServiceMock = Mockito.mockConstruction(HSMServiceImpl::class.java)
        try {
            ...
            LifecycleTest {
                ...
                val association = mock<HSMAssociationInfo> {
                    on { masterKeyAlias } doReturn "alpha"
                    on { tenantId } doReturn "123456789012"
                }
                ...
                val cryptoProcessor = CryptoProcessorImpl(
                    ...
                    hsmStore = mock {
                        on { associate(any(), any(), any(), any()) } doReturn association
                    },
