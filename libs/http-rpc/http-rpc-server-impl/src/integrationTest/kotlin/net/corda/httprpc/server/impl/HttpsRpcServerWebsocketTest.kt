...
class HttpsRpcServerWebsocketTest : HttpRpcServerTestBase() {
    ...
    fun `check WebSocket interaction`() {
        val wsClient = WebSocketClient(HttpClient(SslContextFactory.Client(true)))
        wsClient.start()
        ...
        val uri = URI(
            "wss://${httpRpcSettings.address.host}:${httpRpcSettings.address.port}/" +
