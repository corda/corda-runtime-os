...
class PostgresDbSetup(
    ...
    override fun run() {
        if (!dbInitialised()) {
            ...
            initDb()
            runDbMigration()
            initConfiguration(CordaDb.RBAC.persistenceUnitName, "rbac_user_$dbName", "rbac_password", "$dbUrl?currentSchema=RBAC", DbPrivilege.DML)
            initConfiguration(CordaDb.Crypto.persistenceUnitName, "crypto_user_$dbName", "crypto_password", "$dbUrl?currentSchema=CRYPTO", DbPrivilege.DML)
            initConfiguration(CordaDb.VirtualNodes.persistenceUnitName, dbAdmin, dbAdminPassword, dbUrl, DbPrivilege.DDL)
    ...
    private fun configConnection() =
        OSGiDataSourceFactory.create(DB_DRIVER, dbAdminUrl + "&currentSchema=CONFIG", dbAdmin, dbAdminPassword).connection
    ...
    private fun initConfiguration(connectionName: String, username: String, password :String, jdbcUrl:String, privilege: DbPrivilege) {
