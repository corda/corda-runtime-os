...
class FindAllRequestHandler @Activate constructor(
    @Reference(service = PersistenceManager::class)
    private val persistenceManager: PersistenceManager
) : FlowRequestHandler<FlowIORequest.FindAll> {

    override val type = FlowIORequest.FindAll::class.java

    override fun getUpdatedWaitingFor(context: FlowEventContext<Any>, request: FlowIORequest.FindAll): WaitingFor {
    ...
    override fun postProcess(context: FlowEventContext<Any>, request: FlowIORequest.FindAll): FlowEventContext<Any> {
        val findAllRequest = FindAll(request.className, 0, 0)
        val checkpoint = context.checkpoint
        val entityRequest =
            EntityRequest(Instant.now(), checkpoint.flowId, checkpoint.holdingIdentity.toAvro(), findAllRequest)
        return context.apply {
            checkpoint.persistenceState = persistenceManager.processMessageToSend(request.requestId, entityRequest)
        }
    }
}
