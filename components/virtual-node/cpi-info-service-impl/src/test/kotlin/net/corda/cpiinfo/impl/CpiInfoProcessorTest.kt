...
class CpiInfoProcessorTest {
    private lateinit var processor: CpiInfoProcessor
    ...
    fun beforeEach() {
        ...
        processor = CpiInfoProcessor({ /* don't care about the callback */ }, { /* don't care about callback */ })
    }

    private fun sendOnNextRandomMessage(processor: CpiInfoProcessor): CPI.Identifier {
    ...
    fun `clear message processor`() {
        val processor = CpiInfoProcessor({ /* don't care about callback */ }, { /* don't care about callback */ })
        val cpiIdentifier = CPI.Identifier.newInstance("abc", UUID.randomUUID().toString(), secureHash)
        val cpiMetadata = CPI.Metadata.newInstance(cpiIdentifier, secureHash, emptyList(), "group policy")
    ...
    fun `internal onSnapshot callback is called`() {
        ...
        val processor = CpiInfoProcessor({ onSnapshot = true }, { /* don't care about callback */ })
        val cpiIdentifier = CPI.Identifier.newInstance("abc", UUID.randomUUID().toString(), secureHash)
        val cpiMetadata = CPI.Metadata.newInstance(cpiIdentifier, secureHash, emptyList(), "group policy")
    ...
    fun `internal onError callback is called`() {
        var onError = false
        val processor = CpiInfoProcessor({ /* don't care */ }, { onError = true })
        val cpiIdentifier = CPI.Identifier.newInstance("abc", UUID.randomUUID().toString(), secureHash)
