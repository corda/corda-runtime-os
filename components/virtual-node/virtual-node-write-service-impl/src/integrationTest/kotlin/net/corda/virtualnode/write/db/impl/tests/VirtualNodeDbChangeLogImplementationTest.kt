...
"""
    ...
    private final fun doMigration(includeElement: String, failureText: String? = null) {
        ...
        val primary =
            CpkDbChangeLog(CpkDbChangeLogIdentifier(
                SecureHashImpl(
                    "SHA-256","abc".toByteArray()),"migration/db.changelog-master.xml"), primaryContent)
        val secondary =
            CpkDbChangeLog(CpkDbChangeLogIdentifier(
                SecureHashImpl(
                    "SHA-256","abc".toByteArray()),"migration/dogs-migration-v1.0.xml"), secondaryContent)
