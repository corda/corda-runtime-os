    ...
    override fun call(requestBody: ClientRequestBody): String {
        ...
        try {
            val unconsumedStates = ledgerService.findUnconsumedStatesByExactType(
                Asset::class.java, 10, Instant.now()).results.map { it.state.contractState }

            return jsonMarshallingService.format(
                unconsumedStates.map { AssetTO(it.owner.toString().substringAfter("[")
                    .substringBefore("]"), it.assetName, it.assetId )})
