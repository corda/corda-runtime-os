...
fun <K : Any, V : Any> CordaConsumerRecord<K, V>.toRecord(): Record<K, V> {
    return Record(this.topic, this.key, this.value, this.context)
}

fun <K : Any, V : Any> CordaConsumerRecord<K, V>.toEventLogRecord(): EventLogRecord<K, V> {
    return EventLogRecord(this.topic, this.key, this.value, this.partition, this.offset, this.context)
}

fun Record<*, *>.toCordaProducerRecord(): CordaProducerRecord<*, *> {
    return CordaProducerRecord(this.topic, this.key, this.value, this.context)
}
