...
class AMQPSerializationFormatDecoder(
    ...
    override fun decode(stream: InputStream, recurseDepth: Int, originalBytes: ByteArray, optionalAMQP: Boolean): DecodedBytes {
        ...
        val rendering = mutableMapOf(
            "_value" to readObject(envelope.obj, registry, recurseDepth, false, optionalAMQP),
    ...
    private fun readObject(obj: Any?, registry: DynamicDescriptorRegistry, depth: Int, described: Boolean, optionalAMQP: Boolean): Any? {
        return if (obj is DescribedType) {
            ...
            val transformedObj = readObject(obj.described, registry, depth + 1, true, optionalAMQP)
            ...
            (
                if (serializedBytes != null) {
                    mapOf("_class" to (transformed as Map<*, *>)["_class"], "_value" to recurse(serializedBytes, depth + 1, optionalAMQP))
                } else {
                    transformed
                }
                ).also {
            ...
            return list.map { readObject(it, registry, depth + 1, false, optionalAMQP) }.also {
                if (!described) referencedObjects.add(it)
            }
        } else if (obj is Map<*, *>) {
            return obj.map {
                readObject(it.key, registry, depth + 1, false, optionalAMQP) to readObject(
                    it.value,
                    registry,
                    depth + 1,
                    false,
            ...
            if (bytes.size > 5 && bytes.subSequence(0, 5) == Encoding.cordaMagic) {
                try {
                    return recurse(bytes, depth + 1, optionalAMQP)
                } catch (e: Exception) {
                    // Ignore
                }
            } else {
            ...
            return obj.map { readObject(it, registry, depth + 1, false, optionalAMQP) }.toTypedArray().also {
                if (!described) referencedObjects.add(it)
            }
        } else {
            return obj
