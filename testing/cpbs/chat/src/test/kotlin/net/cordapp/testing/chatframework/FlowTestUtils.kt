...
fun validateProtocol(from: Flow, to: ResponderFlow) {
    ...
    if (annotationOfInitiating.protocol != annotationOfInitiatedBy.protocol) {
        fail(
            "Flow ${from::class.java.name} initiates protocol '${annotationOfInitiating.protocol}'" +
 ...
 */
fun executeConcurrently(vararg blocks: Runnable) {
    blocks.map { Thread { it.run() } }.onEach { it.start() }.onEach { it.join() }
}
