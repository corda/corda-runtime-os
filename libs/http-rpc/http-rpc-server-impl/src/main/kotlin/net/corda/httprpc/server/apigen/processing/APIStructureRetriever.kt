...
class APIStructureRetriever(private val opsImplList: List<PluggableRPCOps<*>>) {
        ...
        try {
            ...
            return this.apply {
                ...
                if (result.errors.isNotEmpty()) {
                    throw IllegalArgumentException("Errors when validate resource classes:\n${result.errors.joinToString("\n")}")
                }
    ...
    private fun Method.toPOSTEndpoint(annotation: HttpRpcPOST): Endpoint {
        ...
        val responseBody = when {
            this.returnsDurableCursorBuilder() && !this.isFiniteDurableStreamsMethod()-> {
                ResponseBody(annotation.responseDescription, DurableReturnResult::class.java, this.toClassAndParameterizedTypes().second)
            }
            this.isFiniteDurableStreamsMethod() -> {
                ResponseBody(annotation.responseDescription, FiniteDurableReturnResult::class.java, this.toClassAndParameterizedTypes().second)
            }
            else -> {
                ResponseBody(annotation.responseDescription, this.toClassAndParameterizedTypes().first, this.toClassAndParameterizedTypes().second)
            }
        }
    ...
    private fun Method.retrieveParameters(includeContextParam: Boolean = false): List<EndpointParameter> {
        try {
            ...
            val contextParam = ParametersTransformerFactory
                    .create("context", DurableStreamContext::class.java.toEndpointParameterParameterizedType()!!)
                    .transform()
