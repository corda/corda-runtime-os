...
class DelegatedSignatureTest {
    ...
    private val verifierService = mock<SigningService>()
    private val publicKey = mock<PublicKey>()
    private val serviceMap = mutableMapOf(publicKey to verifierService)
    ...
    private inner class SignatureTestService(
        ...
        override fun newInstance(constructorParameter: Any?): Any {
            return DelegatedSignature(defaultHash, serviceMap)
        }
    }
    ...
    @Test
    fun `engineInitVerify fail for unknown public key`() {
    ...
    @Test
    fun `engineVerify calls the correct service`() {
        val hash = SigningService.Hash.SHA512
        val data = "data".toByteArray()
        val sign = "sign".toByteArray()
        val signature = Signature.getInstance("test-$hash")
        ...
        verify(verifierService).verify(publicKey, hash, data, sign)
    }

    @Test
    fun `engineVerify without hash will throw an exception`() {
        val data = "data".toByteArray()
        val sign = "sign".toByteArray()
        val signature = Signature.getInstance("test-null")
        signature.initVerify(publicKey)
    ...
    @Test
    fun `engineVerify with unexpected public key will throw an exception`() {
        val data = "data".toByteArray()
        val sign = "sign".toByteArray()
        val signature = Signature.getInstance("test-null")
        signature.initVerify(publicKey)
        signature.update(data)
        serviceMap.remove(publicKey)
