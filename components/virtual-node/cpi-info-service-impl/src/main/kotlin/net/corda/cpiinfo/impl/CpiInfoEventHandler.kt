...
/**
 * Handle the events inside the [CpiInfoService] component.
 *
 * Message processing is in the processor, and the lifecycle is in the component.
 */
class CpiInfoEventHandler(
    private val configurationReadService: ConfigurationReadService,
    private val cpiInfoProcessor: CpiInfoProcessor,
    ...
    /** Stop subscriptions (but not the processor - this is _stop_ not _close_).  */
    ...
    private fun resubscribe(event: ConfigChangedEvent) {
        log.debug { "Cpi Info Service (re)subscribing" }
        subscription?.close()
        subscription = subscriptionFactory.createCompactedSubscription(
            SubscriptionConfig(GROUP_NAME, Schemas.CPI_INFO_TOPIC),
            cpiInfoProcessor,
            event.config
        )
        subscription?.start()
