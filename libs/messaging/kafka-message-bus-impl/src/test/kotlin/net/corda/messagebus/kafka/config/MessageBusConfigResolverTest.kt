...
class MessageBusConfigResolverTest {
    ...
    fun `config resolution for producers`(role: ProducerRoles, expectedProperties: Properties) {
        ...
        val resolver = MessageBusConfigResolver(smartConfigFactory)
        val (_, properties) =
            resolver.resolve(messageBusConfig, ProducerConfig(CLIENT_ID, INSTANCE_ID, role))
    ...
    fun `no transactional id is present if the instance id is not set`() {
        ...
        val resolver = MessageBusConfigResolver(smartConfigFactory)
        val (_, properties) =
            resolver.resolve(messageBusConfig, ProducerConfig(CLIENT_ID, null, ProducerRoles.PUBLISHER))
    ...
    fun `configuration resolution fails if the bus is the incorrect type`() {
        ...
        assertThrows<CordaMessageAPIConfigException> {
            resolver.resolve(messageBusConfig, ProducerConfig(CLIENT_ID, 1, ProducerRoles.DURABLE))
        }
    }
