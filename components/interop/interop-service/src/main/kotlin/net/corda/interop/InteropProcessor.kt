...
class InteropProcessor(
    ...
    private val interopAvroDeserializer: CordaAvroDeserializer<InteropMessage> = cordaAvroSerializationFactory.createAvroDeserializer({},
    ...
    private fun getOutputRecord(
        ...
        val flowMapperEvent = sessionAvroDeserializer.deserialize(payload.array())

        //TODO SessionInit vs SessionData
        val sessionEvent: Any? = flowMapperEvent?.payload
        if (sessionEvent == null) {
            logger.warn("Fail to converted interop message to facade request: empty payload")
            return null
        }
        val facadeRequest = InteropMessageTransformer.getFacadeRequest(
            interopAvroDeserializer.deserialize((sessionEvent as SessionInit).payload.array())!!)
        logger.info("Processing message from flow.interop.event with subsystem $SUBSYSTEM. Key: $key, " +
                    "facade request: $facadeRequest, header $header.")
        ...
        return generateAppMessage(key, flowMapperEvent)
    }
