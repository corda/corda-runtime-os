 ...
 */
class MapSerializer(private val declaredType: ParameterizedType, factory: LocalSerializerFactory) :
    AMQPSerializer<Any> {
    ...
    companion object {
        ...
        private val supportedTypes: Map<Class<out Map<*, *>>, MapCreationFunction> = Collections.unmodifiableMap(
            linkedMapOf(
                // Interfaces
        ...
        fun resolveDeclared(
            ...
            if (supportedTypeIdentifiers.contains(declaredTypeInformation.typeIdentifier.erased)) {
                return if (!declaredTypeInformation.isErased) {
                    declaredTypeInformation
                } else {
                    declaredTypeInformation.withParameters(
            ...
            throw NotSerializableException(
                "Cannot derive map type for declared type " + declaredTypeInformation.prettyPrint(false)
            )
        }
        ...
        fun resolveActual(
            ...
            return when (declaredTypeInformation.typeIdentifier) {
                ...
                else -> erasedInformation.withParameters(
                    LocalTypeInformation.Unknown,
                    LocalTypeInformation.Unknown,
                    sandboxGroup
                )
    ...
    override fun readObject(
        ...
        val entries: Iterable<Pair<Any?, Any?>> =
            (obj as Map<*, *>).map { readEntry(serializationSchemas, metadata, input, it, context) }
...
private fun Class<*>.checkWeakHashMap() {
    if (WeakHashMap::class.java.isAssignableFrom(this)) {
        throw IllegalArgumentException(
            "Weak references with map types not supported. Suggested fix: " +
