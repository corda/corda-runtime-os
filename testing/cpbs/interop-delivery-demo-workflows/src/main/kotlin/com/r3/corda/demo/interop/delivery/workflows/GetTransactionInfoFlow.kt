...
import com.r3.corda.demo.interop.delivery.states.DehydratedDeliveryState
...
private class TransactionInfo(
    val inputStates: List<DehydratedDeliveryState>,
    val outputStates: List<DehydratedDeliveryState>
)
...
class GetTransactionInfoFlow : ClientStartableFlow {
    ...
    private fun getTransactionInfo(transaction: UtxoLedgerTransaction): GetTransactionInfoFlowResult {
        val inputStates = try {
            transaction.getInputStateAndRefs(DeliveryState::class.java).map { it.state.contractState.dehydrate() }
        } catch (e: Exception) {
        ...
        val outputStates = try {
            transaction.getOutputStateAndRefs(DeliveryState::class.java).map { it.state.contractState.dehydrate() }
        } catch (e: Exception) {
            return GetTransactionInfoFlowResult(
