...
class DefaultCryptoServiceTests {
    ...
    fun `Should generate key pair and be able to sign and verify for all supported schemes`(
        ...
        assertTrue(getAllRows().any { it.alias == "$memberId:$alias" })
    ...
    private fun getAllRows(): List<DefaultCryptoPersistentKeyInfo> {
        return cryptoMocks.defaultPersistentKeyCache.data.values.map {
            it.second
        }
    }
