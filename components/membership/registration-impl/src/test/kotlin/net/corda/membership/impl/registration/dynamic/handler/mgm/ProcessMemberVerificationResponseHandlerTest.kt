...
class ProcessMemberVerificationResponseHandlerTest {
    ...
    private val expectedRegistrationTopicKey = "${member.toCorda().shortHash}-${member.groupId}"
    ...
    private fun invokeTestFunction(
        ...
        regState, Record(TOPIC, "${member.toCorda().shortHash}-${member.groupId}", RegistrationCommand(regCommand))
    )
