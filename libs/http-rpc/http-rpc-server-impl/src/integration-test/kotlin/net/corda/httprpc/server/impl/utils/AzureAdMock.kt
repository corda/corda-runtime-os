...
object AzureAdMock {
    ...
    fun generateUserToken(): String {
        val jwt = SignedJWT(JWSHeader.Builder(JWSAlgorithm.RS256).keyID(keyId).build(), JWTClaimsSet.Builder().audience(clientId)
                .issuer(issuer)
                .expirationTime(Date.from(Instant.now().plus(1, ChronoUnit.HOURS)))
                .claim("preferred_username", username)
                .build()).apply { sign(signer) }
    ...
    fun create(): Closeable {
        val streamHandler = TestURLStreamHandlerFactory(mapOf(oidcMetadataUrl to oidcMetadata,
