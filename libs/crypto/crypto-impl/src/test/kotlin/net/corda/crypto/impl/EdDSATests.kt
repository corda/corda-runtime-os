...
import net.corda.v5.base.util.hexToByteArray
...
class EdDSATests {
    ...
    fun `PureEdDSA Ed25519 test vectors`() {
        ...
        assertNotEquals(
            ...
            doSign(
                privateKey,
                testVectorEd25519ctx.messageToSignHex.hexToByteArray()
            ).toHex().toLowerCase()
        )
    }
    ...
    private fun doSign(privateKey: PrivateKey, clearData: ByteArray): ByteArray {
        val signature = Signature.getInstance(EdDSAEngine.SIGNATURE_ALGORITHM, EdDSASecurityProvider())
