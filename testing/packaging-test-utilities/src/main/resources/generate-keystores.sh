...
# reproduce https://r3-cev.atlassian.net/browse/CORE-13267 and make sure we support extended X500 attributes when calculating signer summary hash.
keytool -genkeypair -keyalg EC -alias alice -dname 'CN=Alice, OU=R3, O=Corda, L=Dublin, C=IE, OID.1.3.6.1.4.1.311.60.2.1.3=GB, OID.2.5.4.15=GC, SERIALNUMBER=10103259' \
...
keytool -certreq -alias bob -keystore bob.p12 -storepass ${STOREPASS} \
        | keytool -gencert -alias ca2 -rfc -keystore ca2.p12 -storepass ${STOREPASS} -validity 3650 \
