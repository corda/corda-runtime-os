...
class HSMConfigurationClientComponentTests {
    ...
    fun `Should put HSM`() {
        ...
        val result = sender.act {
            component.putHSM(request)
        }
        val command = assertOperationType<PutHSMCommand>(result)
        assertSame(request, command.config)
        assertSame(request.info, command.config.info)
        assertSame(request.serviceConfig, command.config.serviceConfig)
