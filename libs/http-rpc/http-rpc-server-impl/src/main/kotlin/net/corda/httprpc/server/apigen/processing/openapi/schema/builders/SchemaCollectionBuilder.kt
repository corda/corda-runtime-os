...
internal class SchemaCollectionBuilder(private val schemaModelProvider: SchemaModelProvider) : SchemaBuilder {
    ...
    override fun build(clazz: Class<*>, parameterizedClassList: List<GenericParameterizedType>): SchemaModel = SchemaCollectionModel(
            items = parameterizedClassList.singleOrNull()?.let { schemaModelProvider.toSchemaModel(ParameterizedClass(it.clazz, it.nestedParameterizedTypes)) } ?: SchemaModel(type = DataType.OBJECT)
    )
}
