...
data class Amount<T : Any>(val quantity: Long, val displayTokenSize: BigDecimal, val token: T) : Comparable<Amount<T>> {
    ...
    companion object {
        ...
        /**
         * Build an Amount from a decimal representation. For example, with an input of "12.34 GBP", returns an amount with a quantity of
         * "1234" tokens. The function [getDisplayTokenSize] is used to determine the conversion scaling, for example bonds might be in
         * nominal amounts of 100, currencies in 0.01 penny units.
         *
         * @param displayQuantity The [BigDecimal] to generate the [Amount]'s value from.
         * @param token The token of the [Amount].
         * @param rounding The rounding mode placed on the value.
         ...
         */
        @JvmStatic
        fun <T : Any> fromDecimal(displayQuantity: BigDecimal, token: T, rounding: RoundingMode): Amount<T> {
         ...
         * nominal amounts of 100, currencies in 0.01 penny units.
         *
         * [RoundingMode.FLOOR] will be applied to the [Amount]'s value.
         *
         * @param displayQuantity The [BigDecimal] to generate the [Amount]'s value from.
         * @param token The token of the [Amount].
         * @param T The type of the [token].
         ...
         */
        @JvmStatic
        fun <T : Any> fromDecimal(displayQuantity: BigDecimal, token: T): Amount<T> {
            return fromDecimal(displayQuantity, token, RoundingMode.FLOOR)
        }
 ...
 */
data class SourceAndAmount<T : Any, out P : Any>(val source: P, val amount: Amount<T>, val ref: Any?) {

    constructor(source: P, amount: Amount<T>) : this(source, amount, ref = null)
}
...
class AmountTransfer<T : Any, P : Any>(val quantityDelta: Long,
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun <T : Any, P : Any> fromDecimal(displayQuantityDelta: BigDecimal, token: T, source: P, destination: P): AmountTransfer<T, P> {
            return fromDecimal(displayQuantityDelta, token, source, destination, RoundingMode.DOWN)
        }
     ...
     */
    fun apply(balances: List<SourceAndAmount<T, P>>, newRef: Any?): List<SourceAndAmount<T, P>> {
     ...
     * @throws ArithmeticException if there is underflow in the summations.
     */
    fun apply(balances: List<SourceAndAmount<T, P>>): List<SourceAndAmount<T, P>> = apply(balances, newRef = null)
}
