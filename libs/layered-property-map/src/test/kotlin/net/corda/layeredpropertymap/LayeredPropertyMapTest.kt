...
class LayeredPropertyMapTest {
        ...
        private fun createLayeredPropertyMapImpl() = LayeredPropertyMapImpl(
            mapOf(
                ...
                *convertDummySetValues().toTypedArray(),
                ...
                "singlePublicKeyHash" to "SHA-256:${toHexString("single".toByteArray().sha256Bytes())}",
                "setPublicKeyHash.0" to "SHA-256:${toHexString("set0".toByteArray().sha256Bytes())}",
                "setPublicKeyHash.1" to "SHA-256:${toHexString("set1".toByteArray().sha256Bytes())}",
                "setPublicKeyHash.2" to "SHA-256:${toHexString("set2".toByteArray().sha256Bytes())}"
            ),
            PropertyConverter(
                mapOf(
    ...
    fun `converter functions should work for custom converter of single value`() {
        ...
        val single1 = propertyMap.parse<SecureHash>("singlePublicKeyHash")
        val single2 = propertyMap.parseOrNull<SecureHash>("singlePublicKeyHash")
        assertEquals(single1, single2)
        assertEquals("SHA-256:${toHexString("single".toByteArray().sha256Bytes())}", single1.toString())
        val set = propertyMap.parseSet<SecureHash>("setPublicKeyHash")
        assertEquals(3, set.size)
        val setContents = set.map { it.toString() }
        assertTrue(setContents.contains("SHA-256:${toHexString("set0".toByteArray().sha256Bytes())}"))
        assertTrue(setContents.contains("SHA-256:${toHexString("set1".toByteArray().sha256Bytes())}"))
        assertTrue(setContents.contains("SHA-256:${toHexString("set2".toByteArray().sha256Bytes())}"))
    }
