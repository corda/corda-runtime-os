 ...
 *
 * [WireTransaction][net.corda.v5.ledger.obsolete.transactions.WireTransaction] has an optional time-window property, which if
...
interface TimeWindow {
    ...
    /** Returns true if and only if the given [instant] is within the time interval of this [TimeWindow]. */
    ...
    private data class Between(override val fromTime: Instant, override val untilTime: Instant) : TimeWindow {
        ...
        override val midpoint: Instant get() = fromTime + Duration.between(fromTime, untilTime).dividedBy(2)
