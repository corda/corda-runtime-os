...
abstract class AbstractDriver {
    ...
    private static KeyPairGenerator getKeyPairGenerator(String schemeName)
    ...
    AbstractDriver(@NotNull String schemeName, Set<MemberX500Name> members) {
        requireNonNull(schemeName);
        network = members == null || members.isEmpty() ? emptyMap() : unmodifiableMap(createNetwork(schemeName, members));
    }
    ...
    public final <R> R let(@NotNull Function<DriverDSL, R> action) {
        checkDriver();
        return action.apply(driver);
    }

    public final void run(@NotNull Consumer<DriverDSL> action) {
        checkDriver();
