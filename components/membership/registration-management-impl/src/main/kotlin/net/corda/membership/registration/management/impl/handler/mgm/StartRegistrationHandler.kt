...
internal class StartRegistrationHandler(
    ...
    private val registrationRequestPersistenr = PersistRegistrationRequestService(clock)
    private val memberInfoGetter = QueryMemberInfoService(cordaAvroSerializationFactory)
    private val memberInfoPersister = PersistMemberInfoService(
        cordaAvroSerializationFactory,
        clock,
        memberInfoFactory,
    )
    private val preAuthTokenGetter = QueryPreAuthTokenService()
    ...
    override fun invoke(state: RegistrationState?, key: String, command: StartRegistration): RegistrationHandlerResult {
        ...
        val outputCommand = try {
            ...
            transactionFactory.transaction(command.destination) { em ->
                logger.info("Persisting the received registration request.")
                registrationRequestPersistenr.persist(
                    em,
                    registrationRequest,
                )
                ...
                validatePreAuthTokenUsage(em, pendingMemberInfo, registrationRequest)
                ...
                val existingMemberInfos = memberInfoGetter.get(
                    em,
                    mgmHoldingId.toAvro(),
                    listOf(pendingMemberHoldingId.toAvro()),
                ).map {
                ...
                validateRoleInformation(em, mgmHoldingId, pendingMemberInfo)

                val signedMemberInfo = PersistentSignedMemberInfo(
                    PersistentMemberInfo(
                        command.destination,
                        pendingMemberInfo.memberProvidedContext.toAvro(),
                        pendingMemberInfo.mgmProvidedContext.toAvro(),
                    ),
                ...
                memberInfoPersister.persist(
                    em,
                    listOf(signedMemberInfo),
                )
            }
    ...
    private fun validateRoleInformation(em: EntityManager, mgmHoldingId: HoldingIdentity, member: MemberInfo) {
        ...
        member.notaryDetails?.let { notary ->
            ...
            validateRegistrationRequest(
                ...
                memberInfoGetter.get(
                    em,
                    mgmHoldingId.toAvro(),
                    listOf(HoldingIdentity(notary.serviceName, member.groupId).toAvro()),
                ).firstOrNull() == null,
            ...
            } ?: throw CordaRuntimeException("Could not read group parameters of the membership group '${member.groupId}'.")
        }
    }
    ...
    private fun validatePreAuthTokenUsage(
        ...
        try {
            registrationRequest.getPreAuthToken(keyValuePairListDeserializer)?.let {
                val result = preAuthTokenGetter.query(
                    em = em,
                    tokenId = it.toString(),
                    ownerX500Name = pendingMemberInfo.name.toString(),
                    statuses = listOf(PreAuthTokenStatus.AVAILABLE),
                )
