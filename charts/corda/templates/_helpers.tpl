...
- name: JAVA_TOOL_OPTIONS
  value: {{ if ( get .Values.workers .worker ).debug.enabled -}}
    ...
    {{- end }} {{ if .Values.kafka.sasl.enabled -}}
      -Djava.security.auth.login.config=/etc/config/jaas.conf
    {{- end }} {{ if .Values.openTelemetry.enabled -}}
      -javaagent:/opt/override/opentelemetry-javaagent-1.15.0.jar -Dotel.resource.attributes=service.name={{ .worker }}
      {{- if .Values.openTelemetry.endpoint }} -Dotel.exporter.otlp.endpoint={{ .Values.openTelemetry.endpoint }} -Dotel.exporter.otlp.protocol={{ .Values.openTelemetry.protocol }}
      {{- else }} -Dotel.metrics.exporter=logging -Dotel.traces.exporter=logging
      {{- end -}}
    {{- end -}}
{{- end }}
