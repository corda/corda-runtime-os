...
class GroupParametersReconcilerTest {
    ...
    private val dbConnectionManager: DbConnectionManager = mock {
        on { createEntityManagerFactory(eq(vnode1.vaultDmlConnectionId), any(), eq(true)) } doReturn emf1
        on { createEntityManagerFactory(eq(vnode2.vaultDmlConnectionId), any(), eq(true)) } doReturn emf2
        on { createEntityManagerFactory(eq(vnode3.vaultDmlConnectionId), any(), eq(true)) } doReturn emf3
    }
    ...
    inner class GetVersionedRecordsTest {
        ...
        fun `processing versioned records stream calls expected functions`() {
            ...
            verify(dbConnectionManager, times(2)).createEntityManagerFactory(any(), any(), any())
