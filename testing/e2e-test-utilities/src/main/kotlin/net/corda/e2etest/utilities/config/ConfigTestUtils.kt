...
fun ClusterInfo.updateConfig(config: String, section: String) {
    return cluster {
        ...
        try {
            ...
            // One solution would be for the config path to return a successful result as soon as it receives confirmation the config
 ...
 * If [expectServiceToBeDown] is set to true it is expected the config path will go down before coming back up with the new config.
 */
