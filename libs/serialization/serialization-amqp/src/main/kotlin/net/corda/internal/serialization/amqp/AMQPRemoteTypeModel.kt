...
class AMQPRemoteTypeModel {
    ...
    fun interpret(serializationSchemas: SerializationSchemas, sandboxGroup: SandboxGroup): Map<TypeDescriptor, RemoteTypeInformation> {
        ...
        val result = byTypeDescriptor.mapValues { (typeDescriptor, typeNotation) ->
            cache.getOrPut(typeDescriptor) {
                interpretationState.run { typeNotation.name.getTypeIdentifier(sandboxGroup).interpretIdentifier(sandboxGroup) }
            }
        }
    ...
    data class InterpretationState(
        val notationLookup: Map<TypeIdentifier, TypeNotation>,
        ...
        fun TypeIdentifier.interpretIdentifier(sandboxGroup: SandboxGroup): RemoteTypeInformation =
            if (this in seen) {
                RemoteTypeInformation.Cycle(this)
            } else {
                withSeen(this) {
        ...
        private fun CompositeType.interpretComposite(identifier: TypeIdentifier, sandboxGroup: SandboxGroup): RemoteTypeInformation {
            ...
            val interfaces = interfaceIdentifiers.mapNotNull { interfaceIdentifier ->
                if (interfaceIdentifier == identifier) {
                    null
                } else {
                    interfaceIdentifier.interpretIdentifier(sandboxGroup)
                }
            }

            return if (isInterface) {
                RemoteTypeInformation.AnInterface(typeDescriptor, identifier, properties, interfaces, typeParameters)
            } else {
                RemoteTypeInformation.Composable(typeDescriptor, identifier, properties, interfaces, typeParameters)
            }
        }
        ...
        ): RemoteTypeInformation = when (identifier) {
