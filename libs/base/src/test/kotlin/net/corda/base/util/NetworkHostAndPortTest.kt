    ...
    fun `constructor requires a valid port`() {
        ...
        listOf(65536, -1).forEach {
            assertThatThrownBy {
                ...
                .hasMessage(NetworkHostAndPort.INVALID_PORT_FORMAT.format(it))
        }
    }
    ...
    fun `parseNetworkHostAndPort works`() {
        ...
        listOf("0:0:0:0:0:0:0:1:1234", ":1234", "example.com:-1").forEach {
            assertThatThrownBy {
                ...
                .hasMessage(NetworkHostAndPort.UNPARSEABLE_ADDRESS_FORMAT.format(it))
        }
        listOf("example.com:", "example.com").forEach {
            assertThatThrownBy {
                NetworkHostAndPort.parse(it)
            }.isInstanceOf(IllegalArgumentException::class.java)
                .hasMessage(NetworkHostAndPort.MISSING_PORT_FORMAT.format(it))
        }
    }
}
