...
class PostgresVaultNamedQueryConverterTest {
    ...
    fun `RightParentheses removes last space to the left if previous token was a keyword`() {
        postgresVaultNamedQueryConverter.convert(output, listOf(IsNull(), RightParentheses()))
        assertThat(output.toString()).isEqualTo(" IS NULL)")
        output.clear()
        postgresVaultNamedQueryConverter.convert(output, listOf(IsNotNull(), RightParentheses()))
